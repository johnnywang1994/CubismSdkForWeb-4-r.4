...
function bezierEvaluateCardanoInterpretation(
  ...
  const x: number = time;
  const x1: number = points[0] && points[0].time;
  const x2: number = points[3] && points[3].time;
  const cx1: number = points[1] && points[1].time;
  const cx2: number = points[2] && points[2].time;
...
function evaluateCurve(
  ...
  for (let i: number = curve.baseSegmentIndex; i < totalSegmentCount; ++i) {
    ...
    const pos = motionData.points.at(pointPosition);
